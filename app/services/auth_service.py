...
from app.core.roles import ROLE_ATTENDANT, ROLE_DEV, normalize_role_name
...
class AuthService:
    ...
    def register_company(self, payload: RegisterCompanyRequest) -> Company:
        ...
        if self.company_repository.get_by_slug(slug):
            raise ValidationError("Slug da empresa já existe")

        dev_role = self.user_repository.get_role_by_name(ROLE_DEV)
        free_plan = self.billing_repository.get_plan_by_code("free")
        if not dev_role or not free_plan:
            raise ValidationError("Base inicial do sistema não foi preparada")
