...
import { Bot, Building2, MessageSquareText, Phone, ShoppingBag, Users2, WalletCards, Waypoints } from "lucide-react";
...
const heroClass =
  "overflow-hidden rounded-[1.8rem] border border-white/10 bg-gradient-to-br from-primary/10 via-white/[0.03] to-emerald-500/10 p-5 lg:p-6";
const cardClass = "h-full border-white/8 bg-white/[0.03]";
const tileClass = "rounded-[1.2rem] border border-white/8 bg-black/20 p-4";
        ...
        <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
            <div className={tileClass}>
              ...
              <p className="mt-1 text-sm text-slate-400">conversas aguardando acao</p>
            </div>
            <div className={tileClass}>
              ...
              <p className="mt-1 text-sm text-slate-400">vendas consolidadas</p>
            </div>
            <div className={tileClass}>
              ...
              <p className="mt-1 text-sm text-slate-400">produtos prontos para venda</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <CardContent className="space-y-3">
            {panel.orders.length ? (
              panel.orders.slice(0, 6).map((order) => (
                <div key={order.id} className={tileClass}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-white">{order.code}</p>
                      <p className="mt-1 text-sm text-slate-300">{order.customer_name ?? "Cliente nao identificado"}</p>
        ...
        <Card className={cardClass}>
          ...
          <CardContent className="space-y-3">
            {panel.ai_context_sources.map((source) => (
              <div key={source.source} className={tileClass}>
          ...
          <CardContent className="space-y-3">
            {panel.inventory_alerts.length ? (
              panel.inventory_alerts.map((item) => (
                <div key={String(item.id)} className="rounded-[1.2rem] border border-amber-500/20 bg-amber-500/10 p-4">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
            <div className={tileClass}>
              ...
              <p className="mt-1 text-sm text-slate-400">contas ativas na base</p>
            </div>
            <div className={tileClass}>
              ...
              <p className="mt-1 text-sm text-slate-400">custo acumulado de operacao</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.16fr_0.84fr]">
          ...
          <CardContent className="space-y-3">
            {panel.company_breakdown.length ? (
              panel.company_breakdown.map((company) => (
                <div key={String(company.company_id)} className={tileClass}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
        ...
        <div className="space-y-5">
          <Card className={cardClass}>
            ...
            <CardContent className="space-y-3">
              <div className={`${tileClass} text-sm text-slate-300`}>
